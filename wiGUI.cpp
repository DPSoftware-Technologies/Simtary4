	...
	void Button::Update(const wi::Canvas& canvas, float dt)
	{
		...
		if (IsEnabled() && dt > 0)
		{
			...
			if (state == DEACTIVATING)
			{
				...
				if (onDragEnd) onDragEnd(args);

				if (pointerHitbox.intersects(hitBox) && !disableClicking)
				{
					// Click occurs when the button is released within the bounds
					// Guard: an unbound callback (e.g. backlog's toggle button before
					// the console is first opened) would throw std::bad_function_call,
					// which is a hard crash because exceptions are disabled.
			...
			if (rightButtonClicked)
			{
				EventArgs args;
				args.clickPos = pointerHitbox.pos;
				if (onRightClick) onRightClick(args);
				Activate();
			}
